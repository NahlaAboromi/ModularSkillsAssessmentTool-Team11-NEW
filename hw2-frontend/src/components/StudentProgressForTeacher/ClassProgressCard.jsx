...
const ClassProgressCard = ({ classData, isDark }) => {
  ...
  // helpers: מספר בטוח + toFixed בטוח
  const toNum = (v) => (Number.isFinite(Number(v)) ? Number(v) : 0);
  const toFixedSafe = (v, d = 1) => toNum(v).toFixed(d);

  // הכנת נתוני גרף בטוחים
    ...
    <div className={`${isDark ? 'bg-slate-700' : 'bg-white'} p-6 rounded-lg shadow-md`}>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-right text-sm">
          ...
          <div className="text-lg text-blue-700 font-bold">
            {toFixedSafe(firstScore, 1)} → {toFixedSafe(latestScore, 1)}
          </div>
          {attempts.length > 1 && improvement !== 0 && (
            <div className={`text-sm mt-1 ${improvement > 0 ? 'text-green-600' : 'text-red-600'}`}>
              {improvement > 0 ? '+' : ''}{toFixedSafe(improvement, 1)} improvement
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-200">
        <div className="flex items-center space-x-4">
          {(() => {
            const level = latestAttempt?.analysisResult?.estimatedDepthLevel || 'Unknown';
            const cls =
              level.includes('Advanced') ? 'bg-green-100 text-green-800' :
              level.includes('Intermediate') ? 'bg-yellow-100 text-yellow-800' :
